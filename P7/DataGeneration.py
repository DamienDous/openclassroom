...
class DataGeneration:
	def __init__(self, files_dict, **kwargs):
		self.__dict__.update(kwargs)
		self.__files_dict = files_dict
	...
	def __get_data_from_idx(self, idx, csv_path, col_id, idx_list):
		column_types_df = pd.read_csv('data/'+csv_path, nrows=1)
		file = open('data'+idx+'/'+csv_path)
		...
		for row in reader:
			if int(float(row[col_id])) in idx_list:
				line = []
				for item in row:
					if item == '':
	...
	def write_idx_data(self, curr_id):
		shutil.rmtree('data/temp/'+str(curr_id), ignore_errors=True)
		os.mkdir('data/temp/'+str(curr_id))

		for key, value in self.__files_dict.items():
			df = self.__get_data_from_idx(
				'', value[0]+'.csv', value[1], [curr_id])

			df.to_csv('data/temp/'+str(curr_id)+'/'+value[0]+'.csv',
					  index_label='SK_ID_CURR', index=False)

			if value[0] == 'bureau':
				bb = self.__get_data_from_idx(
					'', 'bureau_balance.csv', 0, list(df['SK_ID_BUREAU']))
				bb.to_csv('data/temp/'+str(curr_id)+'/bureau_balance.csv',
						  index_label='SK_ID_BUREAU', index=False)
		return

	def get_idx_data(self, curr_idx):
		dataframes = {}
		for key, value in self.__files_dict.items():
			dataframes[value[0]] = self.__get_data_from_idx(
				'/temp/'+str(curr_idx), value[0]+'.csv', value[1], [curr_idx])
			if value[0] == 'bureau':
				dataframes['bureau_balance'] = self.__get_data_from_idx(
					'/temp/'+str(curr_idx),
					'bureau_balance.csv',
					0,
					list(dataframes[value[0]]['SK_ID_BUREAU']))
